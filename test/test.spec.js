...
betterThanBefore.setups([
  function() {
    ...
    gitDummyCommit(['fix(ISSUE-1234): avoid a bug', 'BREAKING CHANGE: The Change is huge.']);
  ...
  it('should work if there is no semver tag', function(done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function(chunk) {
        ...
        console.log('ASDASDASDSAD', chunk);

        expect(chunk).to.include('amazing new module');
        expect(chunk).to.include('ISSUE-1234 avoid a bug');
        expect(chunk).to.include('make it faster');
        expect(chunk).to.include(', closes 1, 2'); // Links are not created
        expect(chunk).to.include('Not backward compatible.');
        expect(chunk).to.include('**ISSUE-1234** The Change is huge.');
